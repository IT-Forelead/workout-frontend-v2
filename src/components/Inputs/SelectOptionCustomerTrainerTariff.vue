...
<template>
  <div class="select-none">
    <label ref="dropdown" class="flex items-center w-full relative dark:bg-gray-900">
      <div v-if="selectedOption"
        class="flex items-center space-x-2 border-none focus:ring-0 outline-0 bg-gray-100 w-full text-lg rounded-lg pl-2 py-2 dark:bg-gray-900">
      ...
      <ul v-if="useDropdownStore().isOpenCustomerTrainerTariffDropDown"
        class="absolute w-full bg-white shadow rounded-b-md z-20 top-12 right-0 divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-600 dark:text-gray-300">
        <li v-for="(customerTrainerTariff, idx) in customerTrainerTariffs" :key="idx" @click="optionClicked(customerTrainerTariff)"
          class="flex items-center space-x-2 hover:bg-gray-200 cursor-pointer p-2 dark:hover:bg-gray-700">
          <UserIcon class="w-7 h-7 rounded-full border border-gray-200 bg-gray-50 p-1 dark:bg-gray-900 dark:border-gray-600" />
