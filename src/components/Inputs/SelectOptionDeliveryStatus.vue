...
const clearSelectedOptionData = () => {
  useDropdownStore().setSelectDeliveryStatusOption('')
}

onClickOutside(dropdown, () => {
  useDropdownStore().closeDeliveryStatusDropDown()
})

const optionClicked = (data) => {
  useDropdownStore().setSelectDeliveryStatusOption(data)
  useDropdownStore().closeDeliveryStatusDropDown()
}
</script>
<template>
  <div class="select-none">
    <label ref="dropdown" class="flex items-center w-full relative">
      ...
      <div @click="useDropdownStore().openDeliveryStatusDropDown()" v-else
        class="border-none bg-gray-100 py-2 w-full text-lg rounded-lg cursor-pointer text-gray-500 pl-2">{{ $t('select')
        }}</div>
      <ChevronRightIcon @click="useDropdownStore().openDeliveryStatusDropDown()" v-if="!selectedOption"
      ...
      <ul v-if="useDropdownStore().isOpenDeliveryStatusDropDown"
