  ...
  <div class="select-none">
    <label ref="dropdown" class="flex items-center w-full relative">
      ...
      <ul v-if="useDropdownStore().isOpenTrainerDropDown"
        class="absolute w-full bg-white shadow rounded-b-md z-20 top-12 right-0 divide-y divide-gray-200 dark:divide-gray-600">
        <li v-for="(trainer, idx) in trainers" :key="idx" @click="optionClicked(trainer)"
          class="flex items-center space-x-2 hover:bg-gray-200 cursor-pointer p-2 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-700">
          <UserIcon class="w-7 h-7 rounded-full border border-gray-200 bg-gray-50 p-1 dark:bg-gray-900 dark:border-gray-600" />
          <div class="capitalize">
