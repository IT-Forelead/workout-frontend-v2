...
const submitProductData = () => {
  if (!submitForm.id) {
    ...
    ProductService.editProduct({
      ...
      productNumber: submitForm.count,
    })
      .then(() => {
        notify.success({
          message: t('productEdited'),
        })
...
<template>
  <div v-if="useModalStore().isEditProductModalOpen"
    ...
    <div class="relative p-4 w-full h-full max-w-xl md:h-auto left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
          <div class="text-xl font-medium">{{ $t('editProduct') }}</div>
        ...
        <div class="flex items-center justify-end p-4 space-x-2 border-t dark:border-gray-600">
          ...
          <button v-if="!isLoading" @click="submitProductData()"
