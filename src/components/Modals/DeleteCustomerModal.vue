        ...
        <div class="py-4 px-6 text-center space-y-6">
          <div>
            ...
            <div class="flex items-center space-x-2">
              ...
              <div class="text-lg text-gray-700 dark:text-gray-300">{{ selectedCustomer?.firstname + ' ' + selectedCustomer?.lastname }}
              </div>
            </div>
            <div class="flex items-center space-x-2">
              <div class="text-base text-gray-500 dark:text-white">{{ $t('phone') }}:</div>
              <div class="text-lg text-gray-700 dark:text-gray-300">{{ selectedCustomer?.phone }}</div>
            </div>
            <div class="flex items-center space-x-2">
              <div class="text-base text-gray-500 dark:text-white">{{ $t('gender') }}:</div>
              <div class="text-lg text-gray-700 dark:text-gray-300">{{ genderTranslate(selectedCustomer?.gender) }}</div>
            </div>
            <div class="flex items-center space-x-2">
              <div class="text-base text-gray-500 dark:text-white">{{ $t('qrcode') }}:</div>
              <div v-if="selectedCustomer?.barcode" class="text-lg text-gray-700 dark:text-gray-300">{{ selectedCustomer?.barcode }}</div>
              <div v-else class="text-sm text-gray-500 uppercase dark:text-gray-300">{{ $t('noQrCodeAvailable') }}</div>
            </div>
            <div class="flex items-center space-x-2">
              <div class="text-base text-gray-500 dark:text-white">{{ $t('createdAt') }}:</div>
              <div class="text-lg text-gray-700 dark:text-gray-300">{{ moment(selectedCustomer?.createdAt).format('DD/MM/YYYY H:mm') }}</div>
            </div>
          </div>
          <h3
