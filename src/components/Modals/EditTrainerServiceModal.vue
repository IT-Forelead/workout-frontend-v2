...
const closeModal = () => {
  ...
  useModalStore().closeEditTrainerServiceModal()
  clearForm()
}

watch(
  () => selectedTrainerService.value,
  (data) => {
    if (data) {
      submitForm.userId = data?.userId
      submitForm.id = data?.id
      submitForm.name = data?.name
      submitForm.price = data?.price
...
const submitTrainerServiceData = () => {
  if (!selectTrainer?.value?.id) {
    ...
    TrainerServiceService.editTrainerService(
      ...
      .then(() => {
        notify.success({
          message: t('trainerServiceEdited'),
        })
      ...
      .catch((err) => {
        notify.error({
          message: t('errorEditingTrainerService'),
        })
        isLoading.value = false
      })
  }
...
<template>
  <div v-if="useModalStore().isEditTrainerServiceModalOpen"
    ...
    <div class="relative p-4 w-full h-full max-w-4xl md:h-auto left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-800 border dark:border-gray-600 ">
        <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
          <div class="text-xl font-medium dark:text-white">{{ $t('editTrainerService') }}</div>
          <button @click="closeModal()"
            class="dark:text-gray-300 dark:bg-gray-900 text-gray-600 bg-gray-100 hover:bg-gray-800 hover:text-gray-300 transition-all duration-300 rounded-full text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
            <XIcon />
          </button>
        </div>
        <div class="p-6">
          <div class="grid gap-5 grid-cols-2">
            <div class="space-y-4">
              <div>
                ...
                <SelectOptionTrainer :userId="submitForm.userId"/>
              </div>
              <div>
                <label class="dark:text-white" for="serviceName">{{ $t('serviceName') }}</label>
                <input v-model="submitForm.name" class="border-none text-gray-500 bg-gray-100 rounded-lg w-full text-lg dark:bg-gray-900 dark:text-gray-300 dark:placeholder-gray-400"
