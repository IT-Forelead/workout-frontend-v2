...
const resetPassword = () => {
  ...
  } else if (phone.value.replace(/([() -])/g, '').length < 13) {
    notify.warning({
      ...
      <div v-else class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full md:w-2/3 px-8 md:px-4">
        ...
        <div class="flex flex-col space-y-6">
          <label for="phone">
            ...
            <input id="phone" v-maska data-maska="+998(##) ###-##-##" v-model="phone" type="text"
              class="dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 dark:placeholder-gray-400 w-full py-2 border border-gray-300 rounded focus:outline-none focus:border-slate-500 hover:shadow"
          ...
          <div v-else @click="resetPassword()"
            class="w-full select-none bg-gray-900 dark:hover:bg-gray-700 hover:bg-gray-800 cursor-pointer py-3 font-light text-white rounded flex items-center justify-center">
            <span>{{ $t('send') }}</span>
          </div>
          <router-link to="/"
            class="text-center font-medium text-indigo-600 cursor-pointer hover:text-indigo-900 dark:text-indigo-500 dark:hover:text-indigo-600">
            {{ $t('backToHome') }}
          </router-link>
        </div>
      </div>
